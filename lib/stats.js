...
module.exports.deriveAdvancedStats = function(playerStatsAt) {
    ...
    return {
        ...
        "tovPct":Math.ceil(turnoverPercentage(Number(defense.tov), Number(fieldGoals.fgAtt), Number(freeThrows.ftAtt))),
        "offRtg":Math.ceil(offensiveRating(Number(fieldGoals.fgMade), Number(freeThrows.ftMade), Number(fieldGoals.fgAtt), Number(freeThrows.ftAtt), Number(defense.tov)))
    }
};
